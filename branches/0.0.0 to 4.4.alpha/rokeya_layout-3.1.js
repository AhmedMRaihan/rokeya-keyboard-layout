...
Keyboard.prototype.handleKeyboardInput = function (oEvent,oSource) {
    ...
    ***************************************/
	...
	// do change on unicodeKey: maybe same vowel or consonant+h type input
	...
	// replace last-character, if its "|(dari)" then skip
	if(prev!="" && prev==keyState.unicodeKey 
	&& (this.global.bd.getFollower(prev, 1) == 2 || this.global.bd.getFollower(prev, 1) == 7 || keyState.unicodeKey=="\u09cd") 
	&& keyState.unicodeKey!="\u0964")
	{		
	...
	// combination for "rab/radical" word, ra er por za-fola
	else if( prev=="\u09B0" && keyState.unicodeKey=="\u09CD\u09AF")
	{
		keyState.unicodeKey= this.global.ZWJ + keyState.unicodeKey;
	}
    // prev not a consonant then change vowel...
	else if (this.global.bd.getFollower(keyState.unicodeKey, 1) == 2 &&
        (this.global.bd.getFollower(prev, 1) == 1 || this.global.bd.getFollower(prev, 1) == 3) == false) {

        keyState.unicodeKey = this.global.bd.getFollower(keyState.unicodeKey, 2);
    }
    // prev is hasanta but current key is not consonant/fola...
    if (prev == "\u09CD" && keyState.unicodeKey.length > 0 &&
        (this.global.bd.getFollower(keyState.unicodeKey, 1) == 1 || this.global.bd.getFollower(keyState.unicodeKey, 1) == 3) == false) {
...
Keyboard.prototype.selectKeyPressed = function () {
	...
	else if (String.fromCharCode(code) == "'" || String.fromCharCode(code) == '"')
	{
		alert("comma"); 
		return {code: code,
        iShouldDealIt: false};
